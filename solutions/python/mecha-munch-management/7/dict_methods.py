...
def read_notes(notes: tuple[str, ...] | list[str]) -> dict[str, int]:
...
def update_recipes(ideas: dict[str, dict[str, int]], recipe_updates: tuple[str, dict[str, int]]) -> dict[str, dict[str, int]]:
...
def send_to_store(cart: dict[str, int], aisle_mapping: dict[str, list[str, bool]]) -> dict[str, list[int, str, bool]]:
    ...
    """
    return {key: [value, *aisle_mapping[key]] for key, value in sorted(cart.items(), reverse=True)}
